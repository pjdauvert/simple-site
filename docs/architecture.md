...
│  ├─ /manage/events  EventsEditor (FEATURE_EVENTS) │
│  │    └─ PUT /api/config/events — writes the draft│
...
The Gallery feature (`FEATURE_GALLERY`) organises media-library images with **tags**: an immutable id (plain alphanumerics plus `-`/`_` — also the collection's URL and i18n-key segment) behind a translatable `displayName`/`description`; items carry any number of tags, and deleting a tag cascades to its references (items stay). Unlike the team/contact it has **no blob of its own**: the gallery lives inside the site config (optional `gallery` attribute), follows the **draft → publish** cycle, and is edited from `/manage/gallery` through the granular `PUT /api/config/gallery` — the endpoint 404s (before auth) while the flag is off, but the stored attribute keeps being served by `GET /api/config`; only its interpretation is gated, client-side. Publicly, `/gallery` browses the whole gallery and `/gallery/tag/<tag>` one tag's collection, in the design's **display mode** — centered list, card grid, masonry mosaic, or team-style alternating rows — with the caption side (above/below/left/right) as a second, list-mode-only design setting — grid always captions below, mosaic shows no caption, alternate places it itself (the Design tab shows a symbolic skeleton preview of the picked design); clicking a shot opens a fullscreen **zoom carousel** (wrap-around arrows + keyboard). The editor is a three-tab page (`/manage/gallery/items` — thumbnail-managed items; `/manage/gallery/tags` — the tag lifecycle; `/manage/gallery/design`), each tab saving over a fresh draft read so they never clobber each other. **Items without an image are never rendered**, and a collection with nothing displayable 404s and leaves the nav. Nothing is activated in the navigation automatically: the gallery menu entry lists every declared tag as a deactivated submenu row the admin toggles and orders from the Menu tab (the entry itself can never join a group — its submenu cannot nest); the general page surfaces the tags as a bar and per-item chips navigating to the collections. Tag names, descriptions, the "all items" label and item captions are translation defaults collected by `collectGalleryI18nEntries` while the flag is on (item keys are positional). See [configuration.md](configuration.md#gallery).

The Events feature (`FEATURE_EVENTS`) manages an agenda of dated events. Like the gallery it has **no blob of its own**: events live inside the site config (optional `events` attribute), follow the **draft → publish** cycle, and are edited from `/manage/events` (two tabs — Events (`/manage/events/list`) and Design, each saving over a fresh draft read) through the granular `PUT /api/config/events` — same gating semantics (endpoint 404s before auth while off; the stored attribute keeps being served). Each event carries an **immutable kebab-case slug** (public URL `/events/<slug>` + i18n-key identity, locked after the first save) and stores its instants as **UTC ISO strings**, rendered and classified in the *visitor's* timezone (the admin edits in their own via native `datetime-local` inputs). The public `/events` agenda **places events automatically from their dates** — next (or ongoing) event featured with a four-rule long-format date sentence, upcoming cards, past events by descending year with configurable visibility (all / none / from a cut-off date) — and `/events/<slug>` renders the full fiche (illustration, date sentence, Google Maps location link, website link, markdown description). Event names/descriptions/locations, the section titles and the markdown section headers are translation defaults collected by `collectEventsI18nEntries` while the flag is on (slug-scoped keys — reorder-proof). See [configuration.md](configuration.md#events).
