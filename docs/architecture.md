...
The public navigation is resolved from the config's optional `menu` (ordered entries referencing config pages and feature pages, each with a visibility toggle, plus one-level `group` submenus — see [configuration.md](configuration.md#menu)) into a tree of **nav nodes** (`resolveNavTree` in `apps/web/src/router/publicMenu.ts`); configs without a `menu` fall back to the pages array order. Group nodes render as a popover on desktop and an accordion (or always-open section) inside the mobile menu; the rendering is **origin-agnostic** — `MenuBar` only reads resolved nodes, so a feature can contribute a dynamically-populated group without any nav change: the gallery does exactly that, resolving its top-level menu entry into a group of its displayable themes (`galleryNavGroup`). Feature pages (the flag-gated public pages shipped by optional features — `/team`, `/contact` and `/gallery` today) register their route/label in `apps/web/src/router/publicMenu.ts` and their routes in `AppRouter` ahead of the catch-all; the pages gate themselves on the runtime flags.
...
The Gallery feature (`FEATURE_GALLERY`) organises media-library images into depth-1 **themes** (the menu-group model: immutable camelCase `themeId` derived from the name at creation, renames only touch the title). Unlike the team/contact it has **no blob of its own**: the gallery lives inside the site config (optional `gallery` attribute), follows the **draft → publish** cycle, and is edited from `/manage/gallery` through the granular `PUT /api/config/gallery` — the endpoint 404s (before auth) while the flag is off, but the stored attribute keeps being served by `GET /api/config`; only its interpretation is gated, client-side. Publicly, `/gallery` is the theme index plus the unthemed items (with no theme, the whole gallery is browsed from the root) and `/gallery/<themeId>` explores one theme in the design's **display mode** — centered list, card grid, masonry mosaic, or team-style alternating rows — with the caption side (above/below/left/right) as a second design setting (the Design tab shows a symbolic skeleton preview of the picked design); clicking a shot opens a fullscreen **zoom carousel** (wrap-around arrows + keyboard). The editor is a two-tab page (`/manage/gallery/themes` — themes and thumbnail-managed images; `/manage/gallery/design`), each tab saving over a fresh draft read so they never clobber each other. **Items without an image are never rendered**, and a theme with no displayable item disappears from the nav and the index alike. A top-level gallery menu entry resolves into a dynamically-populated nav group of its themes; theme names and item captions are translation defaults collected by `collectGalleryI18nEntries` while the flag is on (item keys are positional). See [configuration.md](configuration.md#gallery).
