...
The public navigation is resolved from the config's optional `menu` (ordered entries referencing config pages and feature pages, each with a visibility toggle, plus one-level `group` submenus — see [configuration.md](configuration.md#menu)) into a tree of **nav nodes** (`resolveNavTree` in `apps/web/src/router/publicMenu.ts`); configs without a `menu` fall back to the pages array order. Group nodes render as a popover on desktop and an accordion (or always-open section) inside the mobile menu; the rendering is **origin-agnostic** — `MenuBar` only reads resolved nodes, so a future feature can contribute a dynamically-populated group (e.g. gallery themes) without any nav change. Feature pages (the flag-gated public pages shipped by optional features — `/team` and `/contact` today) register their route/label in `apps/web/src/router/publicMenu.ts` and their routes in `AppRouter` ahead of the catch-all; the pages gate themselves on the runtime flags.
