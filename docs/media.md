...
- **Thumbnails** are capped near their display size (2× for retina): media-library cards `w-400`, picker grid `w-200,h-200`, `MediaUrlField` preview `h-200` (it previously downloaded the original for a 100 px preview), admin list avatars `w-80,h-80`, team portraits `w-480,h-480,fo-face` (face-focused crop), menu-bar logo `h-64`, events editor rows `w-96,h-96`.
...
- **Event imagery** follows the gallery's responsive pattern: agenda cards ship a `srcSet` of 320→960 w buckets with a `sizes` hint derived from the design's column count (lazy + async below the fold); the featured poster and the detail-page header ship 480→1920 w buckets. The featured section's blurred-cover backdrop uses a deliberately tiny `w-480,q-50` rendition — the CSS blur erases any detail a heavier download would carry.
- **Section images** are capped at `w-1600` (content images) and `w-1920,q-80` (full-bleed hero/text backgrounds).
- **Watermark** (gallery only, opt-in per [gallery design](configuration.md#gallery)) — a text layer burnt into the delivered rendition, never into the stored original: `l-text,ie-<base64>,fs-<px>,co-<RRGGBBAA>,w-<px>,ia-<align>,pa-<px>,lfo-<focus>,l-end`. Four ImageKit constraints shape the implementation (verified against the API): the text travels **base64-encoded** so any character is safe; `fs` must be an **absolute number** (arithmetic expressions like `fs-bw_mul_0.05` are rejected), so each rendition computes its own size from its width — which is exactly what keeps the mark proportional across a responsive `srcSet`; the layer opacity parameter is rejected, so the opacity rides in the **color's alpha channel**; and a text layer wider than the image is **clipped at the edge** (most visibly with right anchors), so every layer bounds its text box at **90 % of the rendition** (`w`, wraps beyond), aligns the text toward the anchored side (`ia`) and insets it from the edges with a font-proportional padding (`pa`).
