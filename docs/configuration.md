...
- **Section texts** — **`nextTitle`** / **`upcomingTitle`** / **`pastTitle`** (optional; absent/empty → the app's bundled labels) and **`nextHeader`** / **`upcomingHeader`** / **`pastHeader`** (optional markdown introductions under each heading; absent → nothing rendered). All six are translation defaults under `events.nextTitle` … `events.pastHeader`.
- **`design.agendaPage`** (optional, deviations only) — **`pastEventsMode`** (`all` — default —, `none` hides the past section entirely, `from` keeps past events starting on/after **`pastEventsFromDate`** — a `YYYY-MM-DD` cut-off, e.g. January 1st to show only the current year; the date is kept when the mode changes); **`cardAspectRatio`** (`A4` — the portrait print-poster ratio, default (illustrations are mostly digital versions of printed posters) —, `16:9`, `4:3`, `1:1`); **`columns`** (2–4; absent → 3 — desktop only, narrow screens collapse); **`showLocationOnCards`** (absent → shown; only `false` is stored).
- **`design.eventPage`** (optional) — **`imageAspectRatio`** of the detail page's header illustration (same values; absent → `16:9`).
- **Public rendering** — `/events` renders the three sections with no page heading of its own (a section with nothing to show disappears — in particular the featured section is absent when no event is ongoing or upcoming; nothing at all → 404). The featured block is laid out like a hero whose **background is the illustration itself, abstracted into a blurred cover** (posters of any ratio become a full-bleed backdrop; no illustration → the brand gradient): the sharp poster sits on the left third (contained, never cropped), and the presentation on the right two thirds rests on a **contrasted, slightly transparent panel** (readable over any backdrop) — the **date first**, then the name (linking to the detail page), the location, the whole markdown description, and a **More info** contained button to the event website (new tab; same button on the detail page). The **cards** (upcoming and past alike) are **elevated**, entirely clickable to `/events/<slug>`: the name and date(s) sit **on the illustration** over a frosted (blurred, darkened) strip, and the location — plain text, no nested link — sits under the illustration. The featured block's and detail page's **location** opens a Google Maps search on the address in a new tab (`https://www.google.com/maps/search/?api=1&query=<encoded address>` — the displayed address is the translated one, the query keeps the stored original). Events without an image show a brand-gradient placeholder. An unknown slug → 404.
