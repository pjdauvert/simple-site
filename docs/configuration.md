...
- **Feature entries** point at pages shipped by optional features (`team`, `contact` and `gallery` today; events… later). They only render when the feature's flag is on; entries of disabled features are **kept in the data** (greyed out on the Menu tab) so flipping a flag never loses your ordering.
- **Gallery-tag entries** (`{ "type": "galleryTag", "tag": "…" }`) link one gallery tag's collection page (`/gallery/tag/<tag>`) — the **only** way a collection reaches the navigation: nothing is added when a tag is created (see [Gallery](#gallery)). The label defaults to the tag's `displayName` (custom `menuTitle` overrides it; translation key `gallery.tag.<tag>.menuTitle` — the tag's own). The entry hides with the gallery flag, when its tag is deleted (it is then pruned on the next reconciliation, like a deleted page's entry), or when the collection has nothing displayable. They sit at the top level or inside groups like any leaf.
...
- **Reconciliation** — the shared `reconcileMenu` helper keeps the menu in sync with the pages set and the gallery's tags: entries of deleted pages **and of deleted gallery tags** are pruned (inside groups too), new pages are appended (visible) **at the top level**, and newly-enabled features are appended (hidden) until an admin opts them in; tag entries are **never auto-appended** — linking a collection is an explicit admin action. Groups are kept even when emptied — deleting one is an admin decision. The Menu tab applies it on load; the Pages editor applies it on save. Note that renaming a `pageName` counts as delete + re-add, so that entry returns to the end of the menu with default visibility (leaving any group it was in).

The **Menu** tab of `/manage/site` edits this: reorder with the up/down controls (within a level — a group moves as a block), rename with the pencil control, toggle visibility per entry, create groups with **Add group**, link a gallery collection with **Add collection** (offered while the gallery flag is on and unlinked tags remain; each row's **⋮** menu removes such an entry from the menu without touching the tag), pick the desktop submenu display (*Pop-over* / *Secondary bar* — shown once a group exists), and use each row's **⋮** menu to move an entry into or out of a group, toggle a group's *always expanded on mobile* mode, or delete a group (its children return to the top level, in place). Save with `PUT /api/config/menu` — writes the draft; publish to go live.
...
- **`design.captionPosition`** (optional) — where each item's caption (title + subtitle) sits relative to the image, **in the `list` mode only**: `above`, `below` (default — omitted when stored), `left` or `right`. Side captions stack on mobile (left falls back to above, right to below). The other modes place — or drop — the caption themselves (see `displayMode`); the stored value survives mode switches.
...
- **Public rendering** — `/gallery` browses the **whole** gallery as one list in the design's display mode (tags never partition the root), and `/gallery/tag/<tag>` explores one tag's collection — its displayName as the heading, its description as a markdown introduction, its items in the same display mode. Clicking a shot — whatever the mode — opens the **zoom mode**: the image maximised to what the screen can offer with its title/subtitle kept visible, browsed as a **wrap-around carousel** (left/right arrows, keyboard arrows, **horizontal swipe** on touch screens, Escape/close to return), with a **counter** (`3 / 12`) situating the visitor in the list. An unknown tag, a collection with nothing displayable, or a gallery with nothing displayable → 404.
- **Menu** — nothing joins the navigation automatically. The admin links the gallery root through a `{ "type": "feature", "feature": "gallery" }` entry and each collection through a `{ "type": "galleryTag", "tag": "…" }` entry (see [Menu](#menu)) — at the top level or inside menu groups, exactly like pages.
- **Editing** — `/manage/gallery` (shown while the flag is on) is split into two URL-addressed tabs, mirroring the team page: **Items & tags** (`/manage/gallery/items`) manages the tags — create (the id is validated live and immutable; the displayName defaults to it), rename the displayName, edit the description, delete (with its references — the row states the item count) — and the item list as clickable **thumbnails**: an added item appears as a thumbnail at once (its tag chips below), and clicking one opens its attributes dialog (image picked from the media library when that feature is on, title, subtitle with Translate shortcuts, the item's **tags** as a multi-select, and remove). **Design** (`/manage/gallery/design`) picks the display mode and shows **only the options applicable to it** — caption position in list, columns in the tiled modes, tile ratio and filling in grid; the mode-independent settings (spacing, maximum item width, item frame, watermark) always show. Per the [show-only-applicable-options principle](contributing.md#guidelines), an inapplicable option is hidden — never disabled — and its stored value survives mode switches. The tab renders a symbolic skeleton **preview** of the picked design — solid rectangles for the images, solid bars for the title/subtitle lines — sticky on the right half of the screen (desktop only), with a **landscape/portrait toggle** simulating the visitor's screen (portrait collapses the tiled modes to one column and stacks side captions and alternating rows, mirroring the public responsive rules). The toggle hides **no** design option: orientation is the visitor's context, not a design choice. Both tabs save via `PUT /api/config/gallery`, merging over a fresh draft read so they can't clobber each other — **writes the draft**; publish from the dashboard to go live.
