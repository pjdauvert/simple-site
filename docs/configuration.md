...
- **`design.agendaPage`** (optional, deviations only) — **`pastEventsMode`** (`all` — default —, `none` hides the past section entirely, `from` keeps past events starting on/after **`pastEventsFromDate`** — a `YYYY-MM-DD` cut-off, e.g. January 1st to show only the current year; the date is kept when the mode changes); **`cardAspectRatio`** (`16:9` — default —, `4:3`, `1:1`); **`columns`** (2–4; absent → 3 — desktop only, narrow screens collapse); **`showLocationOnCards`** (absent → shown; only `false` is stored).
- **`design.eventPage`** (optional) — **`imageAspectRatio`** of the detail page's header illustration (same values; absent → `16:9`).
- **Public rendering** — `/events` renders the three sections (a section with nothing to show disappears — in particular the featured section is absent when no event is ongoing or upcoming; nothing at all → 404). The featured block is laid out like a hero: illustration on the left third, the **full presentation** on the right two thirds — name (linking to the detail page), date sentence, location, the whole markdown description, then the website link; every card navigates to `/events/<slug>`; the **location** opens a Google Maps search on the address in a new tab (`https://www.google.com/maps/search/?api=1&query=<encoded address>` — the displayed address is the translated one, the query keeps the stored original); the website link opens in a new tab. Events without an image show a brand-gradient placeholder. An unknown slug → 404.
