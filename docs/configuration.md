...
- **Item frame** (optional, all display modes) — the tiles' chrome: **`design.itemElevation`** (drop shadow, MUI elevation scale 0–24; absent/0 → flat), **`design.itemBorder`** + **`design.itemBorderColor`** (2 px solid border; empty color → the theme's primary), and **`design.itemCornerRadius`** (px, 0–48; 0 = sharp corners; absent → 4 px). Only deviations are stored, and the border color only while the border is on.
