...
- **Public rendering** — `/events` renders the three sections (a section with nothing to show disappears; nothing at all → 404). The featured block links to the detail page (name + button); every card navigates to `/events/<slug>`; the **location** opens a Google Maps search on the address in a new tab (`https://www.google.com/maps/search/?api=1&query=<encoded address>` — the displayed address is the translated one, the query keeps the stored original); the website link opens in a new tab. Events without an image show a brand-gradient placeholder. An unknown slug → 404.
