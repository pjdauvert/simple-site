...
// --- cleaning: mirror draftToTheme (drop empty scalars, omit empty arrays/objects) ---
...
export const HeroSectionEditor: React.FC<SectionEditorProps<'hero'>> = ({ value, onChange }) => {
  ...
  const setContent = (patch: Partial<HeroContent>) => {
    onChange({ ...value, content: cleanContent({ ...value.content, ...patch }) });
  };

  const setDesign = (patch: Partial<HeroDesign>) => {
    const next: HeroSectionProps = { ...value };
    const cleaned = cleanDesign({ ...value.design, ...patch });
    if (cleaned) next.design = cleaned;
