...
const TABS = ['themes', 'design'] as const;
type TabKey = (typeof TABS)[number];

/**
 * Gallery management shell (/manage/gallery, flag-gated): a tab bar (Themes &
 * images / Design) over an <Outlet/>. Each tab is a URL sub-route
...
export const GalleryPage: React.FC = () => {
  ...
  const current: TabKey = TABS.find((t) => pathname.endsWith(`/${t}`)) ?? 'themes';
    ...
    <Box sx={{ p: { xs: 2, sm: 4 } }}>
      ...
      <Tabs value={current} sx={{ borderBottom: 1, borderColor: 'divider', mb: 3 }}>
        <Tab
          ...
          to="themes"
          label={<FormattedMessage id="page.manage.gallery.tab.themes" />}
        />
        <Tab
          value="design"
          component={RouterLink}
