 ...
 */
export const MemberPortrait: React.FC<MemberPortraitProps> = ({ member, size }) =>
  member.photoUrl ? (
    <Box
      component="img"
      src={`${member.photoUrl}?tr=w-480,h-480,fo-face,q-80,f-auto`}
      alt={member.name}
      sx={{ width: size, height: size, borderRadius: '50%', objectFit: 'cover', boxShadow: 3 }}
    />
  ) : (
    <Avatar sx={{ width: size, height: size, fontSize: scale(size, 0.4), bgcolor: 'primary.main' }}>
